...
class Bernstein:
    ...
    def __init__(self, n, k, t: np.ndarray):
        ...
        :param t: knot vector
        """
        assert t.shape[0] == n + k + 1 # 获取数组t长度
        self.n = n
        self.k = k
        self.t = t
        """
        对输入的节点向量 t 进行填充的操作，并将填充后的结果保存到类的实例变量 self.tpad 中
        第一个参数是需要填充的数组
        第二个参数是一个元组，用来指定在每个维度上需要填充的数量。
        第三个参数是填充的方式
        (0., self.t[-1]) 表示在填充的两端使用 0. 和 self.t[-1] 这两个值进行填充    
        """
...
if __name__ == '__main__':
    ...
    k = args.k
    if args.bspline:
        knot = np.linspace(0., 1., n + k + 1) # 创建线性间隔的数字序列
